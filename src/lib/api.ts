...
function mapUser(apiUser: ApiUser): User {
  return {
    id: apiUser.id,
    studentId: Number(apiUser.student_id),
    name: apiUser.name,
    email: apiUser.email,
...
class ApiClient {
  ...
  async getMe(): Promise<User> {
    const apiUser = await this.request<ApiUser>("/auth/me");
  ...
  async getClubs(search?: string): Promise<Club[]> {
    ...
    const query = normalizedSearch
      ...
      : "";
    return this.request<Club[]>(`/clubs${query}`);
  }
  ...
  async createClub(data: ClubUpdateRequest & { name: string }): Promise<Club> {
  ...
  async reorderFormQuestions(clubId: string, questionIds: string[]): Promise<ClubForm> {
    return this.request<ClubForm>(`/clubs/${clubId}/form/questions/reorder`, {
      method: "POST",
  ...
  async updateApplicationStatus(
    ...
    return this.request<AdminApplicationListItem>(`/clubs/${clubId}/applications/${appId}/status`, {
      method: "PATCH",
  ...
  async getClubMembers(clubId: string): Promise<ClubMember[]> {
    return this.request<ClubMember[]>(`/clubs/${clubId}/members`);
  }

  async withdrawClubMember(clubId: string, userId: string): Promise<void> {
    await this.request<void>(`/clubs/${clubId}/members/${userId}/withdraw`, {
      method: "PATCH",
    });
  }

  async transferClubPresident(clubId: string, userId: string): Promise<void> {
    await this.request<void>(`/clubs/${clubId}/members/${userId}/role`, {
      method: "PATCH",
    });
  }
  ...
  async getNotifications(): Promise<NotificationItem[]> {
    return this.request<NotificationItem[]>("/me/notifications");
  }

  async markNotificationRead(notificationId: string): Promise<NotificationItem> {
    return this.request<NotificationItem>(`/me/notifications/${notificationId}/read`, {
      method: "PATCH",
    });
  }

  async markAllNotificationsRead(): Promise<void> {
    await this.request<void>("/me/notifications/read-all", { method: "PATCH" });
  }
}
